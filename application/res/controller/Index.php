...
class Index extends Controller
{
    ...
    public function getasset(){
        ...
        $data = gettablecolslike('t_assets',$re);        
    ...
    public function gtasNew($id){
        //$id = input("get.id");
        $ds = gettableDS('t_assets',['asset_num'=>$id]," `name` 资产名称,model_num 规格型号,`tag_num` 标签号,rkd_num 入库单号,  `value` 设备原值,start_time 启用日期 ,zjnx_year 折旧年限 ,deparment 资产部门,location 存放地点 ,  jsr_name 经手人,zrr_name 责任人, person `使 用 人`,zcyt_usage 资产用途 ,jlxx 计量信息 , `status` 设备状态,zcqdfs 资产取得方式 , ' '  采购合同号,  ysdh_num 验收单号, cgly 采购来源 ,   scgj 生产国家,scsjbxxx 生产商及保修信息,jxsjbxxx 经销商及联系方式  "); 
        $this->assign('list',$ds);
        return $this->fetch();
    }
    ...
    public function getchecksplanreport(){
        ...
        //p($data);
        $rt = getDataMapsToJson('checks_detail',$data,'*','check_date desc',$page,$pagesize,$map2,$keystr);
        //$rt = getdsJson('checks_detail',$data,'*','check_date desc ',$page,$pagesize);
        //$rt = gettable('t_Assets');
        return $rt;           
    }
    ...
    public function SetPrintCommandAndStatus(){
        ...
        foreach ($printlist as $k => $v) {
                ...
                if(mb_strlen($rows['deparment'],"utf-8")<=12){
                    $rows['depsize'] = '15';
                }elseif (mb_strlen($rows['deparment'],"utf-8")<25) {
                    $rows['depsize'] = '10';
                }else{
                    $rows['depsize'] = '08';
                }
    ...
    public function putcheckstask(){
        ...
        if($i>0){
            return rtjson('更新成功，已更新 '.$i.' 条数据.','1');
        }elseif($i==0){
            return rtjson('无可更新的数据。');
        }else{
        	return rtjson('更新数据失败。');
        }
    }
...
//################################################################################################
    ...
    ***/
    public function getAssetImg(){
        ...
        $checkpath = $_SERVER["DOCUMENT_ROOT"].'/asset/'.$imgPath ;
