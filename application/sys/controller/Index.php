...
class Index extends Controller
{
    ...
    */
    ...
    public function putPassword(){
        ...
        $result = $validate->check($re);
        if($result){
            $op = md5($re['OldPassword']);
            if(gettablecol('t_sysuser',['UserID'=>$re['UserID']],"Password")!=$op){
                return json(['ID' => '-1','msg'=>'旧密码输入错误!']);
            }
            $map["UserID"]=preg_replace("/'/", "", $re['UserID']);
                    $mapupdate["Password"] = md5($re['Password']);
    ...
    public function login(){
        ...
        $users=db('sysuser')->where(['Tag'=>'1'])->where('UserID|UserEmail|UserMobile|UserName',$userData['username'])->find();
        // dump($users); die;
        if($users){
